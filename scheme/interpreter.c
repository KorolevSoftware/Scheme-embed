...
static struct token_node* set_define(struct token_node* stream) {
	...
	struct token_node* var_or_lambda = stream->body;
...
static struct token_node* print_func(struct token_node* stream) {
	...
	while (iter) {
		struct token_node* result = execute(iter);

		if (result->my_token.type == tt_integer) {
			printf("%d\n", result->my_token.integer);
		}

		if (result->my_token.type == tt_real) {
			printf("%f\n", result->my_token.real);
		}

		if (result->my_token.type == tt_string) {
			printf("%s\n", result->my_token.string);
		}
...
struct token_node* execute(struct token_node* stream) {
	...
	if (instruction.type == tt_integer or instruction.type == tt_real or instruction.type == tt_string) {
		return stream;
	}
